...
const ImageCropProvider = ({
  ...
  const [croppedAreaPixels, setCroppedAreaPixels] = useState<any>(defaultCroppedAreaPixels);
  ...
  const getProcessedImage = async (): Promise<Blob | null> => {
    if (image && croppedAreaPixels) {
      ...
      if (croppedImage && croppedImage.file) {
        ...
        console.error('Error processing image.'); // Agar croppedImage mavjud emas yoki file mavjud emas bo'lsa, xato chiqarish
        return null;
      }
    } else {
      console.error('Image or cropped area pixels are missing.'); // Agar rasmi yoki qirqilgan maydon to'liq emas bo'lsa, xato chiqarish
      return null;
    }
  };
