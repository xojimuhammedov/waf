...
const MyInput = forwardRef((props: FormInputProps, ref: FormInputRef) => {
  ...
  const generalStyles = [
    'appearance-none dark:bg-bg-form text-white py-xs px-3 text-c-m text-text-base border-0 rounded-sm focus:bg-bg-field focus:ring-bg-brand focus:shadow-border-interactive-active focus:border-bg-brand placeholder-text-muted hover:bg-bg-field-hover disabled:bg-bg-disabled disabled:shadow-base disabled:text-text-disabled dark:text-text-title-dark'
  ];
  ...
  return (
    <div className={twMerge(fullWidth && fullWidthStyles, rootClassName)}>
      {label && (
        <label
          className={twMerge([
            'relative mb-1.5 inline-block text-c-m-p font-medium text-text-base text-white dark:text-text-title-dark',
      ...
      <div className={'relative'}>
        ...
        <input
          ...
          aria-invalid={true}
          {...computedProps}
          style={{ background: 'rgba(32, 32, 32, 1)', color: '#fff' }}
          className={twMerge([
            generalStyles,
            size === 'base' && baseSize,
